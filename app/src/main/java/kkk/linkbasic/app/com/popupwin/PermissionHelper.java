...
public class PermissionHelper {
    ...
    private final static int CAMERA = 103;

    private final static int LOCATION = 104;

    private final static int LOCATIONS = 105;
    ...
    private PermissionModel[] mPermissionModels = new PermissionModel[]{
            ...
            new PermissionModel("摄像头", Manifest.permission.CAMERA, "我们需要摄像头权限进行拍照", CAMERA),
            new PermissionModel("定位权限", Manifest.permission.ACCESS_COARSE_LOCATION, "我们需要定位你的位置获取新订单", LOCATION),
            new PermissionModel("定位权限", Manifest.permission.ACCESS_FINE_LOCATION, "我们需要GPS定位你的位置进行导航", LOCATIONS),
            new PermissionModel("存储空间", Manifest.permission.WRITE_EXTERNAL_STORAGE, "我们需要您允许我们读写你的存储卡，以方便我们临时保存一些数据",
                    WRITE_EXTERNAL_STORAGE_CODE)
    };

    //android 版本判断 7.0调起相机会报错
    public static Uri getOutputUri(String pathname) {
        File fie = new File(pathname);
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
            Uri photoURI = FileProvider.getUriForFile(MyApplication.getInstance(), MyApplication.getInstance().getPackageName() + ".provider", fie);
            return photoURI;
        } else {
            return Uri.fromFile(fie);
        }
    }
